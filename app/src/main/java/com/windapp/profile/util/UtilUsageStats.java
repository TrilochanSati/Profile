 ...
 public static   int loadlistsof = 0;
  static   public List<Applist> getApps(String tRange,int loadOf) {
      ...
      loadlistsof=loadOf;
        ...
        try {
            usageEvents = ((UsageStatsManager) ProfileApp.APP.context.getSystemService(Context.USAGE_STATS_SERVICE)).queryEvents(timeRange[0], timeRange[1]);
        ...
        if (arrayList3.size() > 0) {
            ...
            if (Build.VERSION.SDK_INT >= 23) {
                map = getMobileData((TelephonyManager) ProfileApp.APP.context.getSystemService(Context.TELEPHONY_SERVICE), (NetworkStatsManager) ProfileApp.APP.context.getSystemService(Context.NETWORK_STATS_SERVICE));
                z = true;
            } else {
                map = hashMap;
                z = false;
            ...
            PackageManager packageManager = ProfileApp.APP.context.getPackageManager();
            for (String str4 : hashSet) {
                if (!AppUtil.openable(packageManager, str4)) {
                    ...
                    map = map;
                    packageManager = packageManager;
                    arrayList4.add(new Applist(AppUtil.getPackageIcon(ProfileApp.APP.context, str4), AppUtil.parsePackageName(packageManager, str4), str4, j6, j7, i, (map.size() <= 0 || !map.containsKey(str5)) ? 0L : map.get(str5).longValue()));
                } else {
                    map = map;
                    packageManager = packageManager;
                }
    ...
    private static Map<String, Long> getMobileData(TelephonyManager telephonyManager, NetworkStatsManager networkStatsManager) {
        ...
        HashMap hashMap = new HashMap();
        if (ActivityCompat.checkSelfPermission(ProfileApp.APP.context, "android.permission.READ_PHONE_STATE") == 0) {
            try {
